...
class TestFastPow(unittest.TestCase):
    def test_simple_pow(self):
        params = 16, 21
        self.assertEqual(fast_pow(*params), pow(*params))
            ...
            for power in range(3):
                for mod in range(1, 3):
                    with self.subTest(f"a = {a}, power = {power}, mod = {mod}"):
                        self.assertEqual(fast_pow(a, power, mod), pow(a, power, mod))
...
class TestEgcd(unittest.TestCase):
    def test_ones(self):
        self.assertEqual(extgcd(1, 1), (1, 0, 1))

    def test_simple(self):
        self.assertEqual(extgcd(12, 8), (4, 1, -1))

    def test_simple_another_order(self):
        self.assertEqual(extgcd(8, 12), (4, -1, 1))

    def test_equals(self):
        self.assertEqual(extgcd(7, 7), (7, 0, 1))

    def test_big_nums(self):
        self.assertEqual(extgcd(23894798501898, 23948178468116), (2, 2437250447493, -2431817869532))

    def test_powers_of_2(self):
        self.assertEqual(extgcd(pow(2, 50), pow(3, 50)), (1, -260414429242905345185687, 408415383037561))
