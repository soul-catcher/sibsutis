...
int Compare(char *OriginalDateString, char *DateForCompareString, int n)
{
	for (int i = 0; i < n; i++) {
		if (OriginalDateString[i] > DateForCompareString[i]) return 1;
		if (OriginalDateString[i] < DateForCompareString[i]) return -1;
	}
	return 0;
}

int CheckDate(char *OriginalDate, string DateForCompare)
{
	int result = 0;
	char *OriginalYear = new char[2];
	char *YearForCompare = new char[2];
	for (int i = 6; i < 8; i++)
	{
		OriginalYear[i - 6] = OriginalDate[i];
		YearForCompare[i - 6] = DateForCompare[i];
	}
	if (strncmp(OriginalYear, YearForCompare, 2) > 0)
		return 1; 
	if (strncmp(OriginalYear, YearForCompare, 2) < 0)
		return -1;
	if (strncmp(OriginalYear, YearForCompare, 2) == 0)
	{
		char *OriginalMonth = new char[2];
		char *MonthForCompare = new char[2];
		for (int i = 3; i < 5; i++)
		{
			OriginalMonth[i - 3] = OriginalDate[i];
			MonthForCompare[i - 3] = DateForCompare[i];
		}
		if (strncmp(OriginalMonth, MonthForCompare, 2) > 0)
			return 1;
		if (strncmp(OriginalMonth, MonthForCompare, 2) < 0)
			return -1;
		if (strncmp(OriginalMonth, MonthForCompare, 2) == 0)
		{
			char *OriginalDay = new char[2];
			char *DayForCompare = new char[2];
			for (int i = 0; i < 2; i++)
			{
				OriginalDay[i] = OriginalDate[i];
				DayForCompare[i] = DateForCompare[i];
			}
			if (strncmp(OriginalDay, DayForCompare, 2) > 0)
				return 1;
			if (strncmp(OriginalDay, DayForCompare, 2) < 0)
				return -1;
			if (strncmp(OriginalDay, DayForCompare, 2) == 0)
				return 0;
			delete[]OriginalDay;
			delete[]DayForCompare;
		}
		delete[]OriginalMonth;
		delete[]MonthForCompare;
	}
	delete[]OriginalYear;
	delete[]YearForCompare;
	return result;
}
void dbd_add(Record *data, Vertex *&p) {
    ...
    } else if (CheckDate(data->date, p->data->date) < 0) {
        dbd_add(data, p->left);
        if (vr == 1) {
            if (p->balance == 0) {
                Vertex *q = p->left;
...
void search_in_tree(Vertex *root, const string &key, int &i) {
    if (root) {
        if (CheckDate(root->data->date, key) > 0) {
            search_in_tree(root->left, key, i);
        } else if (CheckDate(root->data->date, key) < 0) {
