...
int compare(const Record &record1, const Record &record2) {
    ...
    for (int i : date_comparate_arr) {
        if (record1.date[i] > record2.date[i]) {
            return -1;
        } else if (record1.date[i] < record2.date[i]) {
            return 1;
        }
    }
    if (strcomp(record1.street, record2.street) > 0) {
        return -1;
    } else if (strcomp(record1.street, record2.street) < 0) {
        return 1;
    }
    return 0;
}
...
int quick_search(Record *arr[], const std::string &key) {
    ...
    while (l < r) {
        int m = (l + r) / 2;
        if (strcomp(&arr[m]->date[6], key, 2) < 0) {
            l = m + 1;
        } else {
            r = m;
        }
    }
    if (strcomp(&arr[r]->date[6], key, 2) == 0) {
        return r;
    }
    return -1;
}
...
void tree(Record *arr[], int n) {
    ...
    int key;
    do {
        key = stoi(prompt("Input search key (home), 0 - exit"));
        print_head();
        i = 1;
...
void search(Record *arr[], int &ind, int &n) {
    string key;
    do {
        key = prompt("Input search key (year)");
    } while (key.length() != 2);
    ...
    if (ind == -1) {
        ...
        int i;
        for (i = ind + 1; strcomp(&arr[i]->date[6], key, 2) == 0; ++i) {}
        n = i - ind;
        show_list(&arr[ind], n);
    }
}
